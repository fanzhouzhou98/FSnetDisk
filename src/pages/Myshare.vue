    ...
    <div v-loading="loading" class="wait" style="height:300px;">
      ...
      <div class="displayContent">
        <el-table :data="userShareList" style="width: 100%"  border>
          ...
          <el-table-column prop="shareType" label="分享类型" align="center" width="120">
            <template slot-scope="scope">
              <el-tag :type="scope.row.shareType === 'open' ? 'primary': 'warning'">
                {{ scope.row.shareType === 'open' ? '公开分享': '私密分享' }}</el-tag>
            </template>
          </el-table-column>
          <el-table-column prop="shareKey" label="密码" align="center" width="120" />
          <el-table-column prop="path" label="分享地址" align="center" width="270">
            <template slot-scope="scope">
              <el-link type="primary" target="_blank" :href="fullShortUrl(scope.row.shareUrl)">{{ fullShortUrl(scope.row.shareUrl) }}</el-link>
            </template>
          </el-table-column>
          <el-table-column prop="updatedAt" label="链接创建时间" sortable align="center" width="180" />
...
export default {
  ...
  methods: {
    ...
    getUserShareList() {
      fileApi.getShareFile({pageSize:this.size,pageNo:this.current,...this.searchForm}).then(res => {
        this.loading = false
